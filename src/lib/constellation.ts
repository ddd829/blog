...
export function place(items: Box[], W: number, H: number, pad: number): void {
  for (let i = 0; i < items.length; i++) {
    ...
    for (let t = 0; t < 400 && !placed; t++) {
      ...
      if (items.slice(0, i).every((b) => !overlapAt(a, x, y, b, pad))) {
        a.x = x;
        a.y = y;
        placed = true;
      }
      ...
      for (let j = i + 1; j < items.length; j++) {
        ...
        if (ox > 0 && oy > 0) {
          ...
          if (ox < oy) {
            ...
            const s = ox / 2;
            if (!frozen?.has(a)) a.x -= dir * s;
            if (!frozen?.has(b)) b.x += dir * s;
            if (iter === 0) { const t = a.vx; a.vx = b.vx; b.vx = t; }
          } else {
            const dir = a.y + a.h / 2 <= b.y + b.h / 2 ? 1 : -1;
            const s = oy / 2;
            if (!frozen?.has(a)) a.y -= dir * s;
            if (!frozen?.has(b)) b.y += dir * s;
            if (iter === 0) { const t = a.vy; a.vy = b.vy; b.vy = t; }
          }
        }
      }
    }
