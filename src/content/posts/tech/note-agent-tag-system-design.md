...
上面三节确立了原则，但"写在文档里的规范"会随时间漂移。真正让它生效的，是把规则**代码化为唯一事实源**，并在构建期强制执行——不合规就直接 fail build。本站（Astro）的落地方式如下，所有规则集中在 `src/lib/tags.ts` 一处。
...
以上数量上限与枚举都是**起点而非教条**，且全部集中在一处可改。第 3 节的渐进原则依然成立：让真实的检索 pain point 驱动枚举与命名空间的演化，而不是先验地一次配全。
